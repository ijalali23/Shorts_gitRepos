...
This repo starts from the faceless-shorts engine (the three tracks below still work exactly as
documented) but ships with no example videos yet — `shorts/`, `ai-shorts/`, and `vox-shorts/`
are empty until the first episode is produced here. See `publishing/registry.json` for the
posting history once episodes start going out.
